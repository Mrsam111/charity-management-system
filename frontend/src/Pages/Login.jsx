...
const Login = () => {
  ...
  const handleTestLogin = () => {
    localStorage.setItem("ch-token", "dev-bypass-token")
    localStorage.setItem("ch-user-email", "admin@gmail.com")
    alert("Test login enabled (dev mode)")
    ...
    <div>
      ...
      <div className='signup-page'>
        ...
        <form onSubmit={handleLogin} className='signup'>
          ...
          {showDevBypass && (
            <>
              <button type="button" className='test-login-btn' onClick={handleTestLogin}>
                Test Login (Bypass Auth)
              </button>
              <span className='test-login-note'>Visible only in development mode</span>
            </>
          )}
